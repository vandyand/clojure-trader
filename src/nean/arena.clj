...
(defn get-wrapped-rindies [instrument xindy-config pop-config granularity ga-config]
  (let [big-stream (dv (streams/get-big-stream instrument granularity (:stream-count ga-config)))
...
(defn get-new-xindies-from-wrapped-rindies [wrapped-rindies xindy-config granularity]
  (let [new-stream (dv (streams/get-big-stream 
                        ...
                        granularity (+ (num-weekend-bars granularity) (* 2 (:max-shift xindy-config)))))]
...
(defn get-position-from-xindies [xindies]
  (let [account-balance (get-account-balance)
        ...
        target-pos (int (* account-balance (stats/mean (map #(-> % :sieve seq last) xindies))))]
    (cond
...
(comment
  ...
  (def ga-config (ga/xindy-ga-config 25 100000 0.95))
  ...
  (def granularities ["M4" "M5" "M10" "M15" "M30" "H1" "H2" "H3"])
  (def account-ids ["101-001-5729740-001" "101-001-5729740-002" "101-001-5729740-003"
                    ...
                    "101-001-5729740-007" "101-001-5729740-008"])
