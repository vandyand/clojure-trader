...
(defn streams->open-stream-dv
  "Takes streams from streams/fetch-formatted-streams"
  [streams]
  (->> streams :inception-streams first (mapv :o) dv))
...
(comment
  ...
  (def open-stream-dv (streams->open-stream-dv (streams/fetch-formatted-streams backtest-config)))
